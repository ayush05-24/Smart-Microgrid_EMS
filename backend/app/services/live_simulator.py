...
from ..config import BATTERY, DATA_DIR, LOCAL_TIMEZONE, MICROGRID
...
class LiveTelemetrySimulator:
    ...
    def _generate_record(self) -> dict[str, Any]:
        ...
        dispatch = self._battery_dispatch(solar_kw, load_kw, tariff)

        grid_kw = max(load_kw - solar_kw - max(dispatch["battery_power_kw"], 0.0) + max(-dispatch["battery_power_kw"], 0.0), 0.0)
        load_shed_kw = 0.0
        if self._override_mode == "island":
            available_supply = solar_kw + max(dispatch["battery_power_kw"], 0.0)
    ...
    def _battery_dispatch(self, solar_kw: float, load_kw: float, tariff: float) -> dict[str, Any]:
        ...
        if self._override_mode == "force_charge":
            ...
            discharge_kw = min(BATTERY.max_discharge_kw, (energy - min_energy) * BATTERY.discharge_efficiency, max(load_kw - solar_kw, 18.0))
            action = "discharge"
            ...
            discharge_kw = min(max(load_kw - solar_kw, 0), BATTERY.max_discharge_kw, (energy - min_energy) * BATTERY.discharge_efficiency)
            action = "discharge"
            power_kw = max(discharge_kw, 0.0)
